...
console.log('traction');
{
  ...
  /* The vehicle's own figures are a separate, larger fiction, and they are the
     ones that decide how it drives. Keeping them on their own names is what
     lets the two checks above go on measuring the ground while these measure
     the machine. */
  check('and the vehicle itself gets rather more than that out of it',
    (r.tractionLimit('drive') / ROVER.mass) > accel * 1.5
      && (r.tractionLimit('boost') / ROVER.mass) > (r.tractionLimit('drive') / ROVER.mass),
    `${(r.tractionLimit('drive') / ROVER.mass).toFixed(2)} driving, ` +
    `${(r.tractionLimit('boost') / ROVER.mass).toFixed(2)} boosting, ` +
    `${(r.tractionLimit('brake') / ROVER.mass).toFixed(2)} braking m/s^2`);

  const acc = new Rover({ lat: 0, lon: 0, ground: flat });
  let t = 0;
  while (acc.speed < 4 && t < 60) { acc.step(1 / 120, { throttle: 1 }); t += 1 / 120; }
  /* This used to assert "several seconds" and got 4.4 s, because the drive was
     clamped to the ground's own 1.27 m/s^2 and a 3.6 kN motor rating was being
     thrown away unused. It is 1.8 s now. Still not a car -- a car does this in
     well under a second -- and still traction-limited, just by the vehicle's
     figure rather than the regolith's. */
  check('getting to 4 m/s takes a second or two, not five',
    t > 0.9 && t < 3.5, t.toFixed(1) + ' s');
  check('and cruise is about ten seconds away',
    (() => { let s = 0; const c = new Rover({ lat: 0, lon: 0, ground: flat });
      while (c.speed < ROVER.speedMax - 0.1 && s < 60) { c.step(1 / 120, { throttle: 1 }); s += 1 / 120; }
      return s > 6 && s < 14; })(), 'to ' + (ROVER.speedMax * 3.6).toFixed(0) + ' km/h');
...
console.log('speed and boost');
{
  ...
  let top = 0;
  for (let i = 0; i < 120 * 25; i++) { b.step(1 / 120, { throttle: 1, boost: true }); top = Math.max(top, b.speed); }
  /* The point of the whole boost rework: it has to be REACHABLE. It used to
     take 8.8 s to climb to a ceiling the heat model withdrew at 11.1 s, so
     about two seconds of it existed and it read as doing nothing at all. */
  check('boost actually reaches its own ceiling',
    top > ROVER.speedBoost - 0.5, (top * 3.6).toFixed(1) + ' km/h');
  check('but the drive heats up', b.boostHeat > 0.75, b.boostHeat.toFixed(2));

  /* Long enough to be worth pressing, and comfortably longer than the 8.2 s it
     takes to get to the top of it. That relation is the actual requirement;
     thirty seconds is just where it landed. */
  let t = 0;
  const h = new Rover({ lat: 0, lon: 0, ground: flat });
  while (h.boostHeat < 1 && t < 90) { h.step(1 / 120, { throttle: 1, boost: true }); t += 1 / 120; }
  check('boost lasts about half a minute before it has to rest',
    t > 22 && t < 40, t.toFixed(1) + ' s');
  let reach = 0;
  const q = new Rover({ lat: 0, lon: 0, ground: flat });
  while (q.speed < ROVER.speedBoost - 0.2 && reach < 60) {
    q.step(1 / 120, { throttle: 1, boost: true }); reach += 1 / 120;
  }
  check('and lasts longer than it takes to get there',
    t > reach * 2, `${t.toFixed(1)} s of boost for a ${reach.toFixed(1)} s climb`);

  /* Once it is too hot the drive refuses, and the limiter walks the speed back
     down to the cruise ceiling. Walked, not snapped: the hard clamp this
     replaced deleted 19.5 m/s in a single frame the instant the ceiling moved,
     which no force in the model accounted for. At 2 m/s^2 that walk takes
     about ten seconds, so the check below is made at eleven.

     It also has to be made BEFORE the thermal reset. The cutout has
     hysteresis, so the drive stays locked out until it has cooled to 0.35,
     which from full is thirteen seconds -- and then boost is available again
     and the speed climbs straight back. A fifteen-second window straddled
     that and read 116 km/h on the way back up, which is the cutout working,
     not failing. */
  drive(h, 11, { throttle: 1, boost: true });
  check('and once it is hot the boost stops working',
    h.boostLocked && h.speed < ROVER.speedMax + 0.5,
    `${h.boostLocked ? 'locked out' : 'NOT locked'}, back to ` + (h.speed * 3.6).toFixed(1) + ' km/h');
  check('and it stays locked out until it has really cooled',
    h.boostHeat > 0.35, h.boostHeat.toFixed(2) + ' of heat left');
  drive(h, 10, { throttle: 0 });
  check('then it resets and will boost again',
    !h.boostLocked, h.boostHeat.toFixed(2) + ' of heat');
  drive(h, 30, { throttle: 0 });
  check('and cools down again when you stop asking', h.boostHeat < 0.3, h.boostHeat.toFixed(2));
}
...
console.log('stopping');
{
  ...
  /* This was "over a hundred metres", and the check under it asserted the
     figure was at least four times the Earth-car equivalent -- both true of
     the honest tyre model, where the brake got the same 1.27 m/s^2 as the
  ...
  check('which is still about twice what a car on tarmac would need',
    ...
    `${d.toFixed(0)} m against ${onEarth.toFixed(0)} m for a car at mu 0.9`);

  /* And braking is now meaningfully stronger than accelerating, which is the
     thing that was actually being complained about: before, the two were the
     same number, so nothing you pressed felt like a brake. */
  const bl = r.tractionLimit('brake'), dl = r.tractionLimit('drive');
  check('the brake is stronger than the throttle',
    bl > dl * 1.2, `${(bl / ROVER.mass).toFixed(2)} against ${(dl / ROVER.mass).toFixed(2)} m/s^2`);
}
...
console.log('steering');
{
  ...
  /* The turning circle, which is the number the complaint was really about.
     Cornering used to draw on the same 1.27 m/s^2 as everything else, so
     authority fell as 1/v and the rover was at its most agile at walking pace:
     220 m of radius at cruise and 739 m on boost, which is not steering, it is
     aiming. `lateralGrip` is its own figure now. */
  const radius = (v, boost) => {
    const c = new Rover({ lat: 0, lon: 0, ground: flat });
    drive(c, 2, { throttle: 0 });
    c.speed = v;
    let yaw = 0;
    const N = 240;
    for (let i = 0; i < N; i++) { c.step(1 / 120, { throttle: 1, steer: 1, boost }); yaw += c.yawRate / 120; }
    const rate = yaw / (N / 120);
    return rate > 1e-9 ? c.speed / rate : Infinity;
  };
  const rCruise = radius(ROVER.speedMax, false);
  const rBoost = radius(ROVER.speedBoost, true);
  check('it can be steered at cruise, not just aimed',
...
console.log('slopes');
{
  ...
  /* The guard on the whole grip rework, and the reason there are four named
     figures rather than one big one.

     The vehicle's drive, brake and cornering limits are all fictional and all
     several times the regolith's own. If those were allowed to decide what a
     SLOPE does, the friction angle would move from atan(0.78) = 38 degrees to
     past 50, a rover would power up a crater wall, and the parking brake would
     hold on ground the soil cannot hold anything on. So past the friction
     angle every one of them falls back to the bare surface limit: the soil is
     the thing that has run out, and no amount of torque vectoring across four
     hubs is a substitute for shear strength.

     Stated as behaviour: a slope you cannot park on is a slope you cannot
     drive up either, however hard you ask. */
  const angle = Math.atan(ROVER.grip) * 180 / Math.PI;
  check('the friction angle is still the one the grip implies',
    Math.abs(angle - 38) < 1.5, angle.toFixed(1) + ' degrees');

  for (const deg of [Math.ceil(angle) + 2, 44]) {
    const up = new Rover({ lat: 0, lon: 0, ground: ramp(deg), heading: 0 });
    drive(up, 20, { throttle: 1, boost: true });
    check(`full throttle and boost will not climb ${deg} degrees`,
      up.speed <= 0.05, up.speed.toFixed(2) + ' m/s');
  }
  /* And below it, the better tyre does show up: a slope it can hold, it can
     also drive up briskly. */
  const climb = new Rover({ lat: 0, lon: 0, ground: ramp(25), heading: 0 });
  drive(climb, 25, { throttle: 1 });
  check('but twenty five degrees it climbs at a fair clip',
    climb.speed > 8, (climb.speed * 3.6).toFixed(0) + ' km/h uphill');
...
console.log('driving somewhere');
{
  ...
  /* Ten kilometres when cruise was 60 km/h; thirteen now that it is 80, which
     is just the ceiling times the time minus the ten seconds spent getting
     there. Worth keeping as a check that distance, heading and the odometer
     still agree with each other over a long run. */
  check('ten minutes of driving covers thirteen kilometres',
    km > 11 && km < 14, km.toFixed(2) + ' km');
